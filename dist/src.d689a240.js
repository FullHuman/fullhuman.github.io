...
var purgecssStargazers = document.querySelector('#purgecss-stargazers');
var purgecssDownloads = document.querySelector('#purgecss-downloads');

// functions
var typeLetter = function typeLetter(letter, time) {
  setTimeout(function () {
    weAre.textContent += letter;
  }, time);
};

var typeSentence = function typeSentence(sentence) {
  var i = 0,
      time = 0;
  var interval = 200;
...
// first visit
var isFirstVisit = localStorage.getItem('isFirstVisit') === null ? true : false;
if (!isFirstVisit) {
...
fetch("https://img.shields.io/github/stars/fullhuman/purgecss.json?label=Stars").then(function (res) {
  return res.json();
}).then(function (res) {
  purgecssStargazers.textContent = res.value + ' stargazers';
}).catch();

fetch("https://img.shields.io/npm/dt/purgecss.json").then(function (res) {
  return res.json();
}).then(function (res) {
...
//# sourceMappingURL=/src.d689a240.map